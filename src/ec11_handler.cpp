  ...
  case 0:
    ...
    case 1:
      ...
      case 2:
        ...
        // Swap the page we are actually standing on, never a hardcoded index.
        // The auto variant carries min/max, the manual variant a single value,
        // so the whole secondary page has to be replaced for the numbers below
        // to refresh.
        matrixCoreManager.swapSecondaryPage(
            matrixCoreManager.getCurrentPageIndex(),
            matrixCoreManager.getCurrentSecondaryIndex(),
            brightnessManager.getAutoMode()
                ? matrixCoreManager.getSecondaryPageSetting1()
                : matrixCoreManager.getSecondaryPageSetting2());
        break;
      case 3:
    ...
    case 4: {
      // Screen setting page.
      // On the 128x64 canvas the same three elements are drawn twice - left
      // half is 0-2, right half is 3-5 - so both copies map to the same
      // action. On the 64x64 canvas only 0-2 exist and 3-5 are never reached.
      // Element 0/3 is the title, 1/4 only moves the selection, 2/5 writes it
      // to NVS and reboots: a new geometry needs a fresh DMA setup.
      int16_t screenElement = matrixCoreManager.getCurrentElementGroupIndex();
      ...
      case 1:
      case 4:
        matrixDmaManager.nextPendingPanelType();
        break;
      case 2:
      case 5:
        if (matrixDmaManager.applyPendingPanelType()) {
          ESP.restart();
        }
        break;
      default:
      ...
      Serial.printf("[screen] cw elem=%d pending=%d panel=%d\n", screenElement,
                    matrixDmaManager.getPendingPanelType(),
                    matrixDmaManager.getPanelType());
      break;
    }
    default:
      break;
...
void handleButtonPressedRotateCCW(EC11 *encoder) {
  ...
  case 0:
    ...
    case 1:
      ...
      case 2:
        brightnessManager.setAutoMode(!brightnessManager.getAutoMode());
        // Same as the CW branch: replace the current secondary page in place.
        matrixCoreManager.swapSecondaryPage(
            matrixCoreManager.getCurrentPageIndex(),
            matrixCoreManager.getCurrentSecondaryIndex(),
            brightnessManager.getAutoMode()
    ...
    case 4: {
      // Screen setting page, mirrored direction of handleButtonPressedRotateCW
      int16_t screenElement = matrixCoreManager.getCurrentElementGroupIndex();
      switch (screenElement) {
      case 0:
      case 3:
      ...
      case 1:
      case 4:
        matrixDmaManager.prevPendingPanelType();
        break;
      case 2:
      case 5:
        if (matrixDmaManager.applyPendingPanelType()) {
      ...
      Serial.printf("[screen] ccw elem=%d pending=%d panel=%d\n", screenElement,
                    matrixDmaManager.getPendingPanelType(),
                    matrixDmaManager.getPanelType());
      break;
    }
