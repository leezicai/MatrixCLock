...
// How long *after* the scheduled instant an alarm may still fire (seconds).
// An alarm is never fired before its trigger time; if checkAndTrigger() is
// called late (busy loop, blocking task, clock jump) it may still catch up
// within this window, otherwise the occurrence is dropped silently.
// Keep this >= the interval at which checkAndTrigger() is called.
#define ALARM_CATCHUP_WINDOW 90

// Below this epoch value the system clock is considered not synced yet
...
private:
  ...
  time_t lastTriggeredTime;               // Trigger time of the last fired task
