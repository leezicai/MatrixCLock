...
void setupNet() {
    ...
    // Credentials live in our own NVS namespace, so there is no reason to let
    // the WiFi stack rewrite its copy on every connect - that is flash wear
    // for nothing on a device that is powered on for months.
    WiFi.persistent(false);

    // The link is held open for the whole session now, so let the stack
    // recover from a dropped AP on its own instead of waiting for a task.
    WiFi.setAutoReconnect(true);

    Serial.println("Network module initialized (persistent link mode).");
}
...
bool attemptWiFiConnectOnce() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // Hold the association but let the radio idle between beacons, and let
        // the stack re-associate by itself if the AP goes away.
        WiFi.setAutoReconnect(true);
        setWiFiLowPower();
        return true;
    } else {
...
void setWiFiLowPower() {
    if (isWiFiConnected()) {
      // Modem sleep: the radio wakes for its DTIM beacon and idles otherwise.
      // The association survives, so nothing has to reconnect before a request.
      WiFi.setSleep(true);
    }
  }
...
bool ensureWiFiConnected() {
    ...
    // setAutoReconnect handles the common case; this is the fallback for when
    // the stack has given up, for example after the AP was down for a while.
    Serial.println("WiFi link lost, re-establishing...");
    if (connectNetWithRetry()) {
        return true;
    }
    return false;
}
