...
// Stock Index Settings Implementation
void MatrixDataManager::setStockEnabled(bool enabled) {
    matrixNvsManager.saveBool(NVS_KEY_STOCK_ENABLE, enabled);
	matrixNvsManager.commit();
}

bool MatrixDataManager::getStockEnabled() {
    return matrixNvsManager.loadBool(NVS_KEY_STOCK_ENABLE, DEFAULT_STOCK_ENABLE);
}

// mode 0: red = gain / green = loss, mode 1: green = gain / red = loss
void MatrixDataManager::setStockColorMode(int mode) {
    matrixNvsManager.saveInt(NVS_KEY_STOCK_COLOR, mode);
	matrixNvsManager.commit();
}

int MatrixDataManager::getStockColorMode() {
    return matrixNvsManager.loadInt(NVS_KEY_STOCK_COLOR, DEFAULT_STOCK_COLOR_MODE);
}
