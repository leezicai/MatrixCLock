...
// A task is only expired once the catch-up window has passed, otherwise a
// settings change happening one second after the trigger instant would delete
// the task before checkAndTrigger() ever gets to see it.
void AlarmManager::removeExpiredTasks() {
  time_t now = time(nullptr);

  for (uint8_t i = 0; i < MAX_ALARM_TASKS; i++) {
    if (taskQueue[i].active &&
        taskQueue[i].triggerTime + ALARM_CATCHUP_WINDOW < now) {
      taskQueue[i].active = false;
    }
  }
}
...
void AlarmManager::updateTaskQueue() {
  ...
  for (uint8_t i = 0; i < MAX_ALARMS; i++) {
    ...
    while (checkTime < next24Hours) {
      ...
      // Never re-schedule the instant we have just rung. Without this guard a
      // rebuild that happens inside the same minute would queue the same
      // occurrence again and the alarm would ring twice.
      if (nextTrigger != lastTriggeredTime) {
...
// The queue is sorted, so only the head has to be looked at.
void AlarmManager::checkAndTrigger() {
  time_t now = time(nullptr);

  // Wall clock not valid yet (no RTC / NTP sync): never ring on a bogus time.
  ...
  const time_t triggerTime = taskQueue[0].triggerTime;

  // Never fire early. The previous implementation used a symmetric window
  // (abs(diff) <= 30), which rang the alarm 30 s *before* the configured
  // minute - a 12:23 alarm started at 12:22:30.
  if (now < triggerTime) {
    return;
  }

  // Consume the task before doing anything else, so a single occurrence can
  // never be handled twice.
  taskQueue[0].active = false;

  const bool tooLate = (now - triggerTime) > ALARM_CATCHUP_WINDOW;
  const bool alreadyFired = (triggerTime == lastTriggeredTime);

  if (tooLate) {
    Serial.printf("Alarm task skipped (too late by %lds): Alarm[%d]\n",
                  (long)(now - triggerTime), taskQueue[0].alarmIndex);
  } else if (alreadyFired) {
    Serial.printf("Alarm task skipped (duplicate): Alarm[%d]\n",
                  taskQueue[0].alarmIndex);
  } else {
  ...
  // Move the consumed task to the back and schedule the next occurrence.
  sortTaskQueue();
  updateTaskQueue();
}
