...
String SetAP::generateHTML() {
    ...
    // Preselect the browser's timezone. The list used to default to 0, and a
    // user who never touched it stored UTC - which silently breaks the market
    // session windows and the intraday chart, since both convert API
    // timestamps using this offset.
    html += "  var tzHours = Math.round(-new Date().getTimezoneOffset() / 60);";
    html += "  if (tzHours < -12) { tzHours = -12; }";
    html += "  if (tzHours > 14) { tzHours = 14; }";
    html += "  var tzSelect = document.getElementById('timezone');";
    html += "  tzSelect.value = String(tzHours);";
    html += "};";
