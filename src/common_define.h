...
// Panel type = the single source of truth for the panel geometry.
// The width / height / chain stored in NVS are derived from it (see matrixDma.cpp).
#define PANEL_TYPE_128X64_X1  0   // one 128x64 panel
#define PANEL_TYPE_64X64_X2   1   // two 64x64 panels chained -> 128x64 canvas
#define PANEL_TYPE_64X64_X1   2   // one 64x64 panel
#define PANEL_TYPE_COUNT      3

// loading
// No trailing semicolons: these are values, not statements. With them any use
// outside a plain assignment (e.g. DURATION_LOADING + 1) fails to compile.
// 结尾不能带分号：它们是值不是语句。带分号后只要不是单纯赋值（比如
// DURATION_LOADING + 1）就编译不过。
